module mod_physicaldata
  use mod_indices, only: ngridshi
   ...
   type(walloc), pointer :: myB0_cell, myB0_face1,myB0_face2, myB0
   ...
   type geoalloc
      ...
      double precision, dimension(:,:), pointer :: surfaceC1,surfaceC2,&
         surface1,surface2
      double precision, dimension(:,:,:), pointer :: dx,x
   end type geoalloc

   type(geoalloc), dimension(ngridshi), target :: pgeo, pgeoCoarse, pgeoCoCo
   type(geoalloc), pointer                     :: mygeo
!$OMP THREADPRIVATE(myB0_cell,myB0_face1,myB0_face2,myB0,mygeo)   
