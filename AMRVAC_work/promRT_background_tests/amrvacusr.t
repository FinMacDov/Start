...
UNIT_DENSITY=runit
punit=2.3d0*nHunit*k_B*Teunit ! 0.317538000000000 erg*cm^-3 !where 1 erg*cm^-3 =0.1 pascals
...
{^C&normvar(b^C_)   = dsqrt(4.0d0*dpi*normvar(pp_)) \} !!this has no units so is a normilized mag feild and is eq 1.9975735761524234 on t=0
normt = UNIT_LENGTH/UNIT_VELOCITY ! t=D/S this is nomrisiled time


!print *, dsqrt(4.0d0*dpi*normvar(pp_)), normvar(pp_), dpi
...
Tch=8.d3/Teunit !!corona temperature
Tco=1.8d6/Teunit !!chromosphere temperature
Tpromin=6.0d3/Teunit !! min prom temp
Tpromax=1.4d4/Teunit !! max prom temp
htra1=0.2d0 !! some distance marker
htra2=1.125d0 !! some distance marker
htra3=2.0d0 !!  some distance marker
ybot=1.75d0 !!
ytop=2.0d0  !!
bsca=1.5d0  !!
...
!! this is for the prom itself
   if(ya(j)>=ybot.and.ya(j)<ytop) then
     ...
     Ta(j)=Tco !Tpromin+(Tpromax-Tpromin)*(ya(j)-ybot)/(ytop-ybot)
     Taext(j)=Tco
   endif
!! this is the temp above the prom
   if(ya(j)>ytop) then
     Ta(j)=Tco !Tpromax+0.5d0*(Tco-Tpromax)*(tanh((ya(j)-htra3)/wtra3)+1.d0)
     Taext(j)=Tco
   endif
...
rhob=ra(na)+res/dr*(ra(na+1)-ra(na)) !! base value
pb=pa(na)+res/dr*(pa(na+1)-pa(na)) !! base value
...
if (mype==0) then
 ...
 write(123,*) jmax, 'ya(ix)     ','pa(ix)    ','paext(ix)    ','ra(ix)    ','raext(ix)    ','Ta(ix)    ','Taext    '
 do ix=1,jmax
...
double precision :: jet_w, jet_h, jet_cx, jet_cy, mid_pt
...
!!add small pressure pulse at base
sigma=0.02d0
...
if (first) then
   if (mype==0) then
      print *, w(ixmin1:ixmax1,1,rho_), w(ixmin1:ixmax1,2,rho_), w(ixmin1:ixmax1,3,rho_), w(ixmin1:ixmax1,4,rho_)
   end if
   first=.false.
end if
...
integer, intent(in)                :: ixI^L,ixO^L
double precision, intent(in)       :: x(ixI^S,1:ndim)
double precision                   :: w(ixI^S,nw+nwauxio),tmp(ixG^T),tmp_alt(ixG^T)
