...
!----------------------------------------------------------------------------
!defaults and usage:
call kracken('cmd','-i amrvac.par -if data -restart -1 -slice 0 -collapse 0 '//'--help .false. -convert .false.')
...
integer :: nxlone1,nxlone2
double precision :: dxlone1,dxlone2
...
namelist /filelist/   filenameout,filenameini,filenamelog, snapshotini,&
   ...
   convert_type,dxfiletype,saveprim,primnames, typeparIO,uselimiter,nwauxio,&
   nocartesian,addmpibarrier, writew,writelevel,writespshift,endian_swap,&
    normvar,normt,level_io,level_io_min,level_io_max, autoconvert,sliceascii,&
   slicenext,collapseNext,collapse_type
...
namelist /stoplist/   itmax,tmax,tmaxexact,dtmin,t,it,treset,itreset,residmin,&
   residmax,typeresid
namelist /methodlist/ wnames,fileheadout,typeadvance, ssplitdust,ssplitdivb,&
   ssplitresis,ssplituser,typesourcesplit,sourceimpl,sourceimplcycle,&
   conduction,TCsaturate,TCphi,ncyclemax,sourceparasts,parastsnu,dimsplit,&
   typedimsplit,typeaxial,typecoord,typefull1,typepred1,typelow1,typelimiter1,&
   mcbeta,typegradlimiter1,flatcd,flatsh,flatppm,loglimit,typelimited,&
   useprimitive,typetvdlf, typetvd,typeentropy,entropycoef,typeaverage,&
    B0field,Bdip,Bquad,Boct,Busr,divbwave,typedivbfix,divbdiff,typedivbdiff,&
   compactres,useprimitiveRel,maxitnr,dmaxvel,typepoly,tvdlfeps,BnormLF,&
   smallT,smallp,smallrho,typegrad,typediv,tolernr,absaccnr,strictnr,fixsmall,&
   strictsmall,strictgetaux,nflatgetaux,strictzero,nxdiffusehllc,&
   typespherical,fixprocess,flathllc, ncool,cmulti,coolmethod,coolcurve,Tfix,&
    smallrhod, dustzero, dustmethod,dustspecies,dusttemp, x1ptms,x2ptms,&
   x3ptms,ptmass,tlow,nwtf,neqpartf
namelist /boundlist/  dixB,typeB,typeghostfill,typegridfill,ratebdflux,&
   internalboundary
namelist /amrlist/    mxnest,nbufferx1,nbufferx2,tol,tolratio,errorestimate,&
    amr_wavefilter,nxlone1,nxlone2,dxlone1,dxlone2,iprob,xprobmin1,xprobmin2,&
   xprobmax1,xprobmax2, skipfinestep,wflags,flags,restrictprimitive,&
   prolongprimitive,coarsenprimitive, typeprolonglimit, amrentropy,logflag,&
   tfixgrid,itfixgrid,ditregrid
namelist /paramlist/  time_accurate, courantpar, dtpar, dtdiffpar, dtTCpar,&
   typecourant, slowsteps, cfrac
!----------------------------------------------------------------------------
...
if(firstprocess .and. snapshotini<0) call mpistop&
   ("Please restart from a snapshot when firstprocess=T")
if(convert) autoconvert=.false.

read(unitpar,savelist)
do ifile=1,nfile
   if(dtsave(ifile)<bigdouble/2.and.oktest) write(unitterm,&
      '(" DTSAVE  for file",i2," =",g12.5)') ifile,dtsave(ifile)
   if(ditsave(ifile)<biginteger.and.oktest) write(unitterm,&
...
if(residmin>=zero) then
   ...
   if(residmin<=smalldouble) call mpistop&
      ("Provide value for residual above smalldouble")
end if
...
if(compactres)then
 if(typeaxial/='slab') call mpistop&
    ("compactres for MHD only in cartesian case")
endif
...
do level=1,nlevelshi
   ...
   if(typefull1(level)=='tvd'.and.typeadvance=='twostep') call &
      mpistop(" tvd is onestep method, reset typeadvance=onestep!")
   if(typefull1(level)=='tvd1'.and.typeadvance=='twostep') call &
      mpistop(" tvd1 is onestep method, reset typeadvance=onestep!")
...
if (typedivbfix/='glm1' .and. typedivbfix/='glm2' .and. typedivbfix/='glm3') &
     call mpistop('using GLM, so typedivbfix should be either glm1 or glm2 &
        (or glm3 with no additional sources)')
...
if (any(typelimiter1(1:nlevelshi)== 'ppm').and.(flatsh.and.typephys&
   =='rho')) then
    call mpistop&
       (" PPM with flatsh=.true. can not be used with typephys='rho'!")
end if
if (any(typelimiter1(1:nlevelshi)== 'ppm').and.(flatsh.and.typephys&
   =='hdadiab')) then
     call mpistop&
        (" PPM with flatsh=.true. can not be used with typephys='hdadiab'!")
end if
if (any(typelimiter1(1:nlevelshi)== 'ppm').and.(flatcd.and.typephys&
   =='hdadiab')) then
     call mpistop&
        (" PPM with flatcd=.true. can not be used with typephys='hdadiab'!")
end if
...
do idim=1,ndim
   ...
   if (periodB(idim).or.aperiodB(idim)) then
      do iw=1,nw
         if (typeB(iw,2*idim-1) .ne. typeB(iw,2*idim)) call &
            mpistop("Wrong counterpart in periodic boundary")
         if (typeB(iw,2*idim-1) /= 'periodic' .and. typeB(iw,2*idim&
            -1) /= 'aperiodic') call mpistop("Each dimension should either &
            have all or no variables periodic, some can be aperiodic")
      end do
   end if
end do
...
if(dxlone1*dxlone2<smalldouble)then
   write(unitterm,*)'Wrong value(s) for level one dx:',dxlone1,dxlone2
   call mpistop("Reset nxlone or dxlone!")
endif
dx(1,1)=dxlone1;dx(2,1)=dxlone2;
...
case (4)
   if (mype==0) write(unitterm,*)&
      "Error estimation is Lohner's original scheme"
case default
...
if(.not.time_accurate) then
  if(residmin<=smalldouble .or. residmax==bigdouble) then
   if (mype==0) write(unitterm,*)&
      "Non time_accurate SS computation needs values residmin and residmax"
   call mpistop("Provide values for residual bounds in stoplist")
  end if
end if

! Warn when too few blocks at start of simulation 
if (mype.eq.0 .and. snapshotini.eq.-1 .and.  floor(dble(nxlone1)&
   /(dble(ixGhi1)-2.0d0*dble(dixB))) * floor(dble(nxlone2)/(dble(ixGhi2)&
   -2.0d0*dble(dixB)))  .lt. npe) then
   call mpistop&
      ('Need at least as many blocks on level 1 as cores to initialize!')
end if
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
   ...
   if (nwaux>0) then
      ...
      call getaux(.true.,pw(igrid)%w,px(igrid)%x,ixGlo1,ixGlo2,ixGhi1,ixGhi2,&
         ixMlo1-1,ixMlo2-1,ixMhi1+1,ixMhi2+1,"write_snapshot")
   endif
   iwrite=iwrite+1
...
if (mype /= 0) then
 ...
 do Morton_no=Morton_start(0),Morton_stop(0)
   ...
   offset=int(size_block_io,kind=MPI_OFFSET_KIND) *int(Morton_no&
      -1,kind=MPI_OFFSET_KIND)
   call MPI_FILE_IWRITE_AT(file_handle,offset,pw(igrid)%w,1,type_block_io,&
       iorequest(iwrite),ierrmpi)
 end do
 ...
 if(npe>1)then
  ...
  do ipe =1, npe-1
   do Morton_no=Morton_start(ipe),Morton_stop(ipe)
     ...
     allocate(pwio(igrid_recv(inrecv))%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw))
...
if (mype /= 0) then
 ...
 do Morton_no=Morton_start(0),Morton_stop(0)
   ...
   write(unitsnapshot) pw(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,1:nw)
 end do
 ! write data communicated from other processors
 if(npe>1)then
  nrecv=(Morton_stop(npe-1)-Morton_start(1)+1)
...
integer :: levmaxini, ndimini, ndirini, nwini, neqparini, nxini1,nxini2
...
if (ixGhi1/=nxini1+2*dixB.or.ixGhi2/=nxini2+2*dixB) then
   if (mype==0) write(*,*) "Error: reset resolution to ",nxini1+2*dixB,nxini2&
      +2*dixB
...
double precision :: dvolume(ixGlo1:ixGhi1,ixGlo2:ixGhi2), volumeflat&
   (1:nlevelshi)
integer          :: numlevels, nx1,nx2, nc, ncells, dit
...
do iigrid=1,igridstail; igrid=igrids(iigrid);
   ...
   do iw=1,nw
      wmean(iw)=wmean(iw)+sum(dvolume(ixMlo1:ixMhi1,ixMlo2:ixMhi2)&
         *pw(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,iw))
   end do
end do
