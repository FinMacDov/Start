...
INTEGER,PARAMETER:: r_=1, phi_=-1, z_=-2
...
INTEGER:: isavet(nfile),isaveit(nfile), nslices, slicedir(nslicemax),&
    collapseLevel
...
integer          :: mxnest, dixB, nbufferx1,nbufferx2
integer          :: node, node_sub, levmin, levmax, levmax_sub
logical          :: skipfinestep, phyboundblock, time_advance

common /nodalr/     rnode(rnodehi,ngridshi), rnode_sub(rnodehi,ngridshi),&
    tol(nlevelshi), tolratio(nlevelshi), dx(ndim,nlevelshi), dt, dtimpl,&
    dt_grid(ngridshi)
common /nodali/     node(nodehi,ngridshi), node_sub(nodehi,ngridshi),&
    nbufferx1,nbufferx2, mxnest, dixB, levmin, levmax, levmax_sub
common /nodall/     skipfinestep, phyboundblock(ngridshi), time_advance
...
common /INTE/ nwtf,neqpartf,kr,lvc,it,itmax,itmin,slowsteps,itsave,itsavelast,&
   ...
   idimtest
common /DOUB/ UNIT_LENGTH, UNIT_DENSITY, UNIT_VELOCITY,eqpar,courantpar,dtpar,&
    dtdiffpar, dtTCpar,t,tmax,dtmin,residmin,residmax,residual,tfixgrid,tsave,&
   tsavelast,dtsave,slicecoord,entropycoef,tvdlfeps, mcbeta,parastsnu, TCphi,&
   divbdiff,smallT,smallp,smallrho,amr_wavefilter,dmaxvel,tolernr,absaccnr,&
   tlow,writespshift,cfrac,smallrhod,cmax_mype, cmax_global,x1ptms,x2ptms,&
   x3ptms,ptmass,ratebdflux,normvar,normt,Xload,Xmemory,time_bc
common /CHAR/ typecourant,typeresid,typeadvance,typelow1,typelimited,&
   ...
   typeprolonglimit,typeentropy,typetvd,typetvdlf,typeaverage,typedimsplit,&
   typeaxial,typecoord,typepoly,typedivbdiff,typedivbfix,typediv,typegrad,&
   coolcurve,coolmethod,dustmethod,dustspecies,dusttemp,typeB,typeghostfill,&
   typegridfill,inifile,filenameout,filenameini,filenamelog,fileheadout,&
   wnames,primnames,wnameslog,typefilelog,convert_type, dxfiletype,&
    collapse_type,teststr
