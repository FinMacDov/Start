...
do iigrid=1,igridstail; igrid=igrids(iigrid)
   allocate(pwio(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw+nwauxio))
   pwio(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)=pw(igrid)%w(ixGlo1:ixGhi1,&
      ixGlo2:ixGhi2,1:nw)
   if(nwauxio>=1)then
     call specialvar_output(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1-1,ixMlo2&
        -1,ixMhi1+1,ixMhi2+1,pwio(igrid)%w,px(igrid)%x,normconv)
   endif
end do

if (saveprim) then
 do iigrid=1,igridstail; igrid=igrids(iigrid)
  call primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1+1,ixGlo2&
     +1,ixGhi1-1,ixGhi2-1,pwio(igrid)%w,px(igrid)%x)
 end do
else
 if (nwaux>0) then
  do iigrid=1,igridstail; igrid=igrids(iigrid)
   call getaux(.true.,pwio(igrid)%w,px(igrid)%x,ixGlo1,ixGlo2,ixGhi1,ixGhi2,&
      ixGlo1+1,ixGlo2+1,ixGhi1-1,ixGhi2-1,"oneblock")
  end do
 end if
end if
 ...
 if (.not.fileopen) then
   ...
   select case(convert_type)
    case("oneblock")
     ...
     write(qunit,*)( (ixMhi1-ixMlo1+1)*(ixMhi2-ixMlo2+1))*(Morton_stop(npe-1)&
        -Morton_start(0)+1),ng1(level_io)*(ixMhi1-ixMlo1+1),ng2(level_io)&
        *(ixMhi2-ixMlo2+1)
     write(qunit,*)t*normt
    case("oneblockB")
   ...
   do ig2=1,ng2(level_io)
       do ig1=1,ng1(level_io)
         ...
         if(B0field) then
           pwio(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,b1_)=pwio(igrid)%w&
              (ixGlo1:ixGhi1,ixGlo2:ixGhi2,b1_)+myB0_cell%w(ixGlo1:ixGhi1,&
              ixGlo2:ixGhi2,1)
           pwio(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,b2_)=pwio(igrid)%w&
              (ixGlo1:ixGhi1,ixGlo2:ixGhi2,b2_)+myB0_cell%w(ixGlo1:ixGhi1,&
              ixGlo2:ixGhi2,2)
           pwio(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,b3_)=pwio(igrid)%w&
              (ixGlo1:ixGhi1,ixGlo2:ixGhi2,b3_)+myB0_cell%w(ixGlo1:ixGhi1,&
              ixGlo2:ixGhi2,3);
         
         end if
        
         where(dabs(pwio(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw&
            +nwauxio))<smalldouble**2)
            pwio(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw+nwauxio)=zero
         endwhere
       end do
       ...
       do ig1=1,ng1(level_io)
         do ix1=ixMlo1,ixMhi1
           ...
           Master_write : if(mype==0) then
             select case(convert_type)
               case("oneblock")
                 write(qunit,fmt="(100(e14.6))") px(igrid)%x(ix1,ix2,1:ndim)&
                    *normconv(0),(pwio(igrid)%w(ix1,ix2,iwrite(iw))&
                    *normconv(iwrite(iw)),iw=1,writenw)
               case("oneblockB")
                 write(qunit) real(px(igrid)%x(ix1,ix2,1:ndim)&
                    *normconv(0)),(real(pwio(igrid)%w(ix1,ix2,iwrite(iw))&
                    *normconv(iwrite(iw))),iw=1,writenw)
             end select
           end if Master_write
         end do
...
Master_cpu_open : if (mype == 0) then
 ...
 write(qunit,"(i7)") ( (ixMhi1-ixMlo1+1)*(ixMhi2-ixMlo2+1) )*(Morton_stop(npe&
    -1)-Morton_start(0)+1)
end if Master_cpu_open

do Morton_no=Morton_start(mype),Morton_stop(mype)
  igrid=sfc_to_igrid(Morton_no)
  if(saveprim) call primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
     ixMhi1,ixMhi2,pw(igrid)%w,px(igrid)%x)
  if (mype/=0)then
      itag=Morton_no
 ...
 if (mype==0) then
  loop_cpu : do ipe =1, npe-1
   loop_Morton : do Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         do ix1=ixMlo1,ixMhi1
            do iw=1,nw
              if( dabs(pw(igrid)%w(ix1,ix2,iw)) < smalldouble ) &
                 pw(igrid)%w(ix1,ix2,iw) = zero
            enddo
...
double precision, dimension(ixMlo1-1:ixMhi1,ixMlo2-1:ixMhi2,nw&
   +nwauxio)   :: wC_TMP
double precision, dimension(ixMlo1:ixMhi1,ixMlo2:ixMhi2,nw+nwauxio)     :: &
   wCC_TMP
...
logical :: fileopen
integer :: iigrid,igrid,nx1,nx2,nxC1,nxC2,ixCmin1,ixCmin2,ixCmax1,ixCmax2,&
   ixCCmin1,ixCCmin2,ixCCmax1,ixCCmax2,iw
character(len=80) :: filename
integer :: filenr
...
if (nleafs==1 .and. mxnest==1) then
  write(qunit) nxC1,nxC2
  write(qunit)eqpar
  write(qunit)tmpnames
else
  write(qunit) -nleafs,-nleafs
  write(qunit) eqpar
  write(qunit)tmpnames
  ...
  do iigrid=1,igridstail; igrid=igrids(iigrid);
     ...
     select case (typeaxial)
      case ("slab","slabtest")
      rnode_IDL(rpxmin1_:rpxmin2_)=rnode(rpxmin1_:rpxmin2_,igrid);
      rnode_IDL(rpxmax1_:rpxmax2_)=rnode(rpxmax1_:rpxmax2_,igrid); 
      case ("cylindrical")
      ...
      case ("spherical")
       rnode_IDL(rpxmin1_)=rnode(rpxmin1_,igrid) *sin(rnode(rpxmin2_,igrid))
       rnode_IDL(rpxmax1_)=rnode(rpxmax1_,igrid) *sin(rnode(rpxmax2_,igrid))
     ...
     write(qunit) rnode_IDL(rpxmin1_)*normconv(0),rnode_IDL(rpxmin2_)&
        *normconv(0),rnode_IDL(rpxmax1_)*normconv(0),rnode_IDL(rpxmax2_)&
        *normconv(0)
  end do
end if
...
do iigrid=1,igridstail; igrid=igrids(iigrid);
  call calc_grid(qunit,igrid,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,ixCmin1,&
     ixCmin2,ixCmax1,ixCmax2,ixCCmin1,ixCCmin2,ixCCmax1,ixCCmax2,.true.)
  select case(convert_type)
  case('idl')
    ! write out corner coordinates and (averaged) corner values
    write(qunit) xC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,1:ndim)*normconv(0)
    do iw=1,nw+nwauxio
      write(qunit) wC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,iw)*normconv(iw)
    end do
  case('idlCC')
    ! write out cell center coordinates and cell center values
    write(qunit) xCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,1:ndim)&
       *normconv(0)
    do iw=1,nw+nwauxio
      write(qunit) wCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,iw)&
         *normconv(iw)
    end do
  end select 
end do
...
integer :: nx1,nx2,nxC1,nxC2,nodesonlevel,elemsonlevel,ixCmin1,ixCmin2,&
   ixCmax1,ixCmax2,ixCCmin1,ixCCmin2,ixCCmax1,ixCCmax2
   ...
   select case(convert_type)
     case('tecplot')
       ...
       write(qunit,"(a,i7,a,a,i7,a,i7,a,f25.16,a,a)") 'ZONE T="',level,'"',&
          ...
          *normt,', DATAPACKING=POINT, ZONETYPE=',  'FEQUADRILATERAL'
       do iigrid=1,igridstail; igrid=igrids(iigrid);
         if (node(plevel_,igrid)/=level) cycle
         call calc_grid(qunit,igrid,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,&
            ixCmin1,ixCmin2,ixCmax1,ixCmax2,ixCCmin1,ixCCmin2,ixCCmax1,&
            ixCCmax2,.true.)
         do ix2=ixCmin2,ixCmax2
         do ix1=ixCmin1,ixCmax1
            x_TEC(1:ndim)=xC_TMP(ix1,ix2,1:ndim)*normconv(0)
            w_TEC(1:nw+nwauxio)=wC_TMP(ix1,ix2,1:nw+nwauxio)*normconv(1:nw&
               +nwauxio)
       ...
       if(nw+nwauxio==1)then
         ...
         write(qunit,"(a,i7,a,a,i7,a,i7,a,f25.16,a,i1,a,a)") 'ZONE T="',level,&
            ...
            *normt,', DATAPACKING=BLOCK, VARLOCATION=([', ndim&
            +1,']=CELLCENTERED), ZONETYPE=',  'FEQUADRILATERAL'
       else
...
integer :: nxCC1,nxCC2,idims,jxCmin1,jxCmin2,jxCmax1,jxCmax2,iwe,iwb1,iwb2,&
   iwb3
...
do ix=ixCCmin1,ixCCmax1
    xCC(ix,ixCCmin2:ixCCmax2,1)=rnode(rpxmin1_,igrid)+(dble(ix-ixCCmin1)&
       +half)*dx1
end do
do ix=ixCCmin2,ixCCmax2
    xCC(ixCCmin1:ixCCmax1,ix,2)=rnode(rpxmin2_,igrid)+(dble(ix-ixCCmin2)&
       +half)*dx2
end do
...
do ix=ixCmin1,ixCmax1
    xC(ix,ixCmin2:ixCmax2,1)=rnode(rpxmin1_,igrid)+dble(ix-ixCmin1)*dx1
end do
do ix=ixCmin2,ixCmax2
    xC(ixCmin1:ixCmax1,ix,2)=rnode(rpxmin2_,igrid)+dble(ix-ixCmin2)*dx2
end do
...
if (nwextra>0) then
 ...
 do idims=1,ndim
  select case(idims)
   case(1)
     jxCmin1=ixGhi1+1-dixB;jxCmin2=ixGlo2;
     jxCmax1=ixGhi1;jxCmax2=ixGhi2;
     do ix1=jxCmin1,jxCmax1
         w(ix1,jxCmin2:jxCmax2,nw-nwextra+1:nw) = w(jxCmin1&
            -1,jxCmin2:jxCmax2,nw-nwextra+1:nw)
     end do 
     jxCmin1=ixGlo1;jxCmin2=ixGlo2;
     jxCmax1=ixGlo1-1+dixB;jxCmax2=ixGhi2;
     do ix1=jxCmin1,jxCmax1
         w(ix1,jxCmin2:jxCmax2,nw-nwextra+1:nw) = w(jxCmax1&
            +1,jxCmin2:jxCmax2,nw-nwextra+1:nw)
     end do 
   case(2)
     jxCmin1=ixGlo1;jxCmin2=ixGhi2+1-dixB;
     jxCmax1=ixGhi1;jxCmax2=ixGhi2;
     do ix2=jxCmin2,jxCmax2
         w(jxCmin1:jxCmax1,ix2,nw-nwextra+1:nw) = w(jxCmin1:jxCmax1,jxCmin2&
            -1,nw-nwextra+1:nw)
     end do 
     jxCmin1=ixGlo1;jxCmin2=ixGlo2;
     jxCmax1=ixGhi1;jxCmax2=ixGlo2-1+dixB;
     do ix2=jxCmin2,jxCmax2
         w(jxCmin1:jxCmax1,ix2,nw-nwextra+1:nw) = w(jxCmin1:jxCmax1,jxCmax2&
            +1,nw-nwextra+1:nw)
     end do 
  end select
 end do
...
if(nwauxio>0)then
  ...
  call specialvar_output(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1-1,ixMlo2-1,ixMhi1&
     +1,ixMhi2+1,w,px(igrid)%x,normconv)
endif
...
if(saveprim.and.first) call primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1&
   -1,ixMlo2-1,ixMhi1+1,ixMhi2+1,w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw),&
   px(igrid)%x)
...
do iw=1,nw+nwauxio
   if (B0field.and.iw>b0_.and.iw<=b0_+ndir) then
      ...
      do ix1=ixCCmin1,ixCCmax1
         wCC(ix1,ix2,iw)=w(ix1,ix2,iw)+pB0_cell(igrid)%w(ix1,ix2,idir)
      end do
      end do
   else
      do ix2=ixCCmin2,ixCCmax2
      do ix1=ixCCmin1,ixCCmax1
          wCC(ix1,ix2,iw)=w(ix1,ix2,iw)
      end do
      end do
   end if
end do
...
if((.not.saveprim) .and. B0field) then
   do ix2=ixCCmin2,ixCCmax2
do ix1=ixCCmin1,ixCCmax1
       wCC(ix1,ix2,iwe)=w(ix1,ix2,iwe) &
           +half*( pB0_cell(igrid)%w(ix1,ix2,iwb1-b0_)**2+pB0_cell(igrid)%w&
              (ix1,ix2,iwb2-b0_)**2+pB0_cell(igrid)%w(ix1,ix2,iwb3-b0_)**2 ) &
           + ( w(ix1,ix2,iwb1)*pB0_cell(igrid)%w(ix1,ix2,iwb1-b0_)&
              +w(ix1,ix2,iwb2)*pB0_cell(igrid)%w(ix1,ix2,iwb2-b0_)&
              +w(ix1,ix2,iwb3)*pB0_cell(igrid)%w(ix1,ix2,iwb3-b0_) )
   end do
end do
endif
...
if(typeaxial=='slab')then
   ...
   do iw=1,nw+nwauxio
      if (B0field.and.iw>b0_.and.iw<=b0_+ndir) then
         ...
         do ix2=ixCmin2,ixCmax2
         do ix1=ixCmin1,ixCmax1
           wC(ix1,ix2,iw)=sum(w(ix1:ix1+1,ix2:ix2+1,iw) +pB0_cell(igrid)%w&
              (ix1:ix1+1,ix2:ix2+1,idir))/dble(2**ndim)
         end do
         end do
      else
        if(uselimiter)then
           ...
           do idims =1,ndim
              ...
              call dwlimiter2(dwC,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixCmin1,ixCmin2,&
                 ixCmax1,ixCmax2,iw,idims,ldw,dxlevel(idims))
              wC(ixCmin1:ixCmax1,ixCmin2:ixCmax2,iw)=w(ixCmin1:ixCmax1,&
                 ixCmin2:ixCmax2,iw)+half*ldw(ixCmin1:ixCmax1,ixCmin2:ixCmax2)
           end do
        else
          do ix2=ixCmin2,ixCmax2
          do ix1=ixCmin1,ixCmax1
   ...
   if((.not.saveprim) .and. B0field) then
      do ix2=ixCmin2,ixCmax2
do ix1=ixCmin1,ixCmax1
         wC(ix1,ix2,iwe)=sum(w(ix1:ix1+1,ix2:ix2+1,iwe) &
           +half*( pB0_cell(igrid)%w(ix1:ix1+1,ix2:ix2+1,iwb1-b0_)**2&
              +pB0_cell(igrid)%w(ix1:ix1+1,ix2:ix2+1,iwb2-b0_)**2&
              +pB0_cell(igrid)%w(ix1:ix1+1,ix2:ix2+1,iwb3-b0_)**2 ) &
           + ( w(ix1:ix1+1,ix2:ix2+1,iwb1)*pB0_cell(igrid)%w(ix1:ix1&
              +1,ix2:ix2+1,iwb1-b0_)+w(ix1:ix1+1,ix2:ix2+1,iwb2)&
              *pB0_cell(igrid)%w(ix1:ix1+1,ix2:ix2+1,iwb2-b0_)+w(ix1:ix1&
              +1,ix2:ix2+1,iwb3)*pB0_cell(igrid)%w(ix1:ix1+1,ix2:ix2&
              +1,iwb3-b0_) ) ) &
            /dble(2**ndim)
      end do
end do
   ...
   do iw=1,nw+nwauxio
      if (B0field.and.iw>b0_.and.iw<=b0_+ndir) then
         ...
         do ix1=ixCmin1,ixCmax1
           wC(ix1,ix2,iw)= sum((w(ix1:ix1+1,ix2:ix2+1,iw)+pB0_cell(igrid)%w&
              (ix1:ix1+1,ix2:ix2+1,idir)) *pgeo(igrid)%dvolume(ix1:ix1&
              +1,ix2:ix2+1))    /sum(pgeo(igrid)%dvolume(ix1:ix1+1,ix2:ix2+1))
         end do
         end do
      else
         do ix2=ixCmin2,ixCmax2
         do ix1=ixCmin1,ixCmax1
           wC(ix1,ix2,iw)=sum(w(ix1:ix1+1,ix2:ix2+1,iw)*pgeo(igrid)%dvolume&
              (ix1:ix1+1,ix2:ix2+1)) /sum(pgeo(igrid)%dvolume(ix1:ix1&
              +1,ix2:ix2+1))
         end do
         end do
      end if
           ...
           + ( w(ix1:ix1+1,ix2:ix2+1,iwb1)*pB0_cell(igrid)%w(ix1:ix1&
              ...
              *pB0_cell(igrid)%w(ix1:ix1+1,ix2:ix2+1,iwb2-b0_)+w(ix1:ix1&
              ...
              +1,iwb3-b0_) ) ) &
                    ...
                    /sum(pgeo(igrid)%dvolume(ix1:ix1+1,ix2:ix2+1))
      end do
end do
   endif
...
if(nocartesian) then
  ...
  xC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,1:ndim)          = &
     xC(ixCmin1:ixCmax1,ixCmin2:ixCmax2,1:ndim)
  wC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,1:nw+nwauxio)    = &
     wC(ixCmin1:ixCmax1,ixCmin2:ixCmax2,1:nw+nwauxio)
  xCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,1:ndim)        &
     = xCC(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,1:ndim)
  wCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,1:nw+nwauxio)  &
     = wCC(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,1:nw+nwauxio)
else
   ...
   case ("cylindrical")
      ...
      do iw=1,nw+nwauxio
         ...
         if (iw0>=0.and.iw<=iw0+ndim.and.iw<=nw) then
            idim=iw-iw0
            w_TEC(iw0+idim)=wC(ix1,ix2,iw0+1)*normal(idim,1)+wC(ix1,ix2,iw0&
               +2)*normal(idim,2)
         else
   ...
   do iigrid=1,igridstail; igrid=igrids(iigrid);
    ...
    if ((rnode(rpxmin1_,igrid)>=xprobmin1+(xprobmax1-xprobmin1)&
       *writespshift(1,1).and.rnode(rpxmin2_,igrid)>=xprobmin2&
       +(xprobmax2-xprobmin2)*writespshift(2,1)).and.(rnode(rpxmax1_,igrid)&
      ...
      select case(convert_type)
       case('vtu')
         ...
         if(iw<=nw) then 
            ...
            write(qunit,'(200(1pe14.6))') ((wC_TMP(ix1,ix2,iw)&
               *normconv(iw),ix1=ixCmin1,ixCmax1),ix2=ixCmin2,ixCmax2)
            write(qunit,'(a)')'</DataArray>'
         enddo
         write(qunit,'(a)')'</PointData>'
       ...
       case('vtuCC')
         ...
         if(iw<=nw) then 
            ...
            write(qunit,'(a,a,a)')'<DataArray type="Float64" Name="',&
               TRIM(wnamei(iw)),'" format="ascii">'
            write(qunit,'(200(1pe14.6))') ((wCC_TMP(ix1,ix2,iw)&
...
'<DataArray type="Int32" Name="connectivity" format="appended" offset="',&
 ...
 do Morton_no=Morton_start(0),Morton_stop(0)
   ...
   do iw=1,nw+nwauxio
     ...
     if(cell_corner) then
       write(qunit) length
       write(qunit) ((real(wC_TMP(ix1,ix2,iw)*normconv(iw)),ix1&
          =ixCmin1,ixCmax1),ix2=ixCmin2,ixCmax2)
     else
       write(qunit) lengthcc
       write(qunit) ((real(wCC_TMP(ix1,ix2,iw)*normconv(iw)),ix1&
...
integer :: iigrid, igrid, level, ngrids, nx1,nx2
...
nx1=ixGhi1-2*dixB;nx2=ixGhi2-2*dixB;
...
do level=levmin,levmax
   ...
   do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      write(qunit,'(a,i5.5,a,3(x,i11))') 'object "pos',ngrids&
         -1, '" class gridpositions counts ',nx1+1,nx2+1
      write(qunit,'(a,3(x,f25.16))') '  origin ',rnode(rpxmin1_,igrid)&
         *normvar(0),rnode(rpxmin2_,igrid)*normvar(0)
      
      
      write(qunit,'(a,2(x,f25.16))') '  delta  ',rnode(rpdx1_,igrid)&
         *normvar(0),0.0d0
      write(qunit,'(a,2(x,f25.16))') '  delta  ',0.0d0,rnode(rpdx2_,igrid)&
         *normvar(0)
      ...
      write(qunit,'(a,i5.5,a,3(x,i11))') 'object "con',ngrids&
         -1, '" class gridconnections counts ',nx1+1,nx2+1
...
do level=levmin,levmax
   do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      offset = offset + size_recsep
      ! write data array
      call varout_dx_condep(qunit,pw(igrid)%w,ixGlo1,ixGlo2,ixGhi1,ixGhi2)
      offset = offset + (nw+nwauxio)*nummeshpoints*size_double + size_recsep
   end do
end do
...
!============================================================================
subroutine varout_dx_condep(qunit,w,ixGmin1,ixGmin2,ixGmax1,ixGmax2)

include 'amrvacdef.f'

integer, intent(in) :: qunit, ixGmin1,ixGmin2,ixGmax1,ixGmax2
double precision, intent(in) :: w(ixGmin1:ixGmax1,ixGmin2:ixGmax2,1:nw)

integer :: ixMmin1,ixMmin2,ixMmax1,ixMmax2, ix1,ix2, iw
!-----------------------------------------------------------------------------
...
write(qunit) (((w(ix1,ix2,iw)*normvar(iw),iw=1,nw),ix2=ixMmin2,ixMmax2),ix1&
   =ixMmin1,ixMmax1)
...
double precision, dimension(ixMlo1-1:ixMhi1,ixMlo2-1:ixMhi2,nw&
   +nwauxio)   :: wC_TMP,wC_TMP_recv
double precision, dimension(ixMlo1:ixMhi1,ixMlo2:ixMhi2,nw+nwauxio)     :: &
   wCC_TMP,wCC_TMP_recv
double precision, dimension(0:nw+nwauxio)                   :: normconv
...
double precision, dimension(ixMlo1-1:ixMhi1,ixMlo2-1:ixMhi2,&
   ndim)         :: xC_TMP
double precision, dimension(ixMlo1:ixMhi1,ixMlo2:ixMhi2,ndim)           :: &
   xCC_TMP
double precision, dimension(ixMlo1-1:ixMhi1,ixMlo2-1:ixMhi2,nw&
   +nwauxio)   :: wC_TMP
double precision, dimension(ixMlo1:ixMhi1,ixMlo2:ixMhi2,nw+nwauxio)     :: &
    ...
    case('vtumpi','pvtumpi')
      ...
      do iw=1,nw+nwauxio
         if(iw<=nw) then 
            ...
            write(qunit,'(a,a,a)')'<DataArray type="Float64" Name="',&
               TRIM(wnamei(iw)),'" format="ascii">'
            write(qunit,'(200(1pe14.6))') ((wC(ix1,ix2,iw)*normconv(iw),ix1&
               =ixCmin1,ixCmax1),ix2=ixCmin2,ixCmax2)
            write(qunit,'(a)')'</DataArray>'
      enddo
      write(qunit,'(a)')'</PointData>'
    ...
    case('vtuCCmpi','pvtuCCmpi')
      ...
      do iw=1,nw+nwauxio
         if(iw<=nw) then 
            ...
            write(qunit,'(a,a,a)')'<DataArray type="Float64" Name="',&
               TRIM(wnamei(iw)),'" format="ascii">'
            write(qunit,'(200(1pe14.6))') ((wCC(ix1,ix2,iw)&
               *normconv(iw),ix1=ixCCmin1,ixCCmax1),ix2=ixCCmin2,ixCCmax2)
            write(qunit,'(a)')'</DataArray>'
      enddo
      write(qunit,'(a)')'</CellData>'
    ...
    case('vtimpi','pvtimpi')
      ...
      do iw=1,nw+nwauxio
         if(iw<=nw) then 
            ...
            write(qunit,'(a,a,a)')'<DataArray type="Float64" Name="',&
               TRIM(wnamei(iw)),'" format="ascii">'
            write(qunit,'(200(1pe20.12))') ((wC(ix1,ix2,iw)&
               *normconv(iw),ix1=ixCmin1,ixCmax1),ix2=ixCmin2,ixCmax2)
            write(qunit,'(a)')'</DataArray>'
      enddo
      write(qunit,'(a)')'</PointData>'
    ...
    case('vtiCCmpi','pvtiCCmpi')
      ...
      do iw=1,nw+nwauxio
         if(iw<=nw) then 
            ...
            write(qunit,'(a,a,a)')'<DataArray type="Float64" Name="',&
               TRIM(wnamei(iw)),'" format="ascii">'
            write(qunit,'(200(1pe20.12))') ((wCC(ix1,ix2,iw)&
               *normconv(iw),ix1=ixCCmin1,ixCCmax1),ix2=ixCCmin2,ixCCmax2)
            write(qunit,'(a)')'</DataArray>'
      enddo
      write(qunit,'(a)')'</CellData>'
       ...
       if(nw+nwauxio==1)then
        ...
        if(ndim+nw+nwauxio<10) then
         ...
         if (mype==0.and.(nodesonlevelmype>0.and.elemsonlevelmype&
            ...
            +nwauxio,']=CELLCENTERED), ZONETYPE=',  'FEQUADRILATERAL'
        else
         if (mype==0.and.(nodesonlevelmype>0.and.elemsonlevelmype&
     ...
     case('tecplotmpi')
        ...
        do  Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         do ix1=ixrvCmin1,ixrvCmax1
              ...
              w_TEC(1:nw+nwauxio)=wC_TMP_RECV(ix1,ix2,1:nw+nwauxio)&
     ...
     case('tecplotCCmpi')
       ...
       if(nw+nwauxio==1)then
         ...
         if(nodesonlevelmype>0.and.elemsonlevelmype>0) write(qunit,&
            ...
            ', N=',nodesonlevelmype,', E=',elemsonlevelmype,&
             ', SOLUTIONTIME=',t*normt,', DATAPACKING=BLOCK, VARLOCATION=([',&
             ndim+1,']=CELLCENTERED), ZONETYPE=',  'FEQUADRILATERAL'
       else
        if(ndim+nw+nwauxio<10) then
         ! difference only in length of integer format specification for ndim+nw+nwauxio
         if(nodesonlevelmype>0.and.elemsonlevelmype>0) write(qunit,&
   ...
   do Morton_no=Morton_start(mype),Morton_stop(mype)
    ...
    if (node(plevel_,igrid)/=level) cycle
      ...
      if ((rnode(rpxmin1_,igrid)>=xprobmin1+(xprobmax1-xprobmin1)&
         *writespshift(1,1).and.rnode(rpxmin2_,igrid)>=xprobmin2&
         +(xprobmax2-xprobmin2)*writespshift(2,1)).and.(rnode(rpxmax1_,igrid)&
         <=xprobmax1-(xprobmax1-xprobmin1)*writespshift(1,2)&
         .and.rnode(rpxmax2_,igrid)<=xprobmax2-(xprobmax2-xprobmin2)&
         *writespshift(2,2))) then
        call calc_grid(qunit,igrid,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,&
           ixCmin1,ixCmin2,ixCmax1,ixCmax2,ixCCmin1,ixCCmin2,ixCCmax1,&
           ixCCmax2,.true.)
        do iw=1,nw
